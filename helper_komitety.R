# http://w1.c1.rada.gov.ua/pls/site2/p_komity_list?pidid=3005
...
# Функція для скачування членства в комітетах 
get_komitet <- function(){
  ...
  for(i in comit_id) {
    ...
    write.table(results,
                paste0("members_komit_rada09_",  Sys.Date(),".csv"), 
...
membership_k <- read.csv("members_komit_rada09_2021-06-09.csv", header = FALSE, sep=";") %>% # Change a date according to a saved file
  rename("full_name_k"=V1, 
         "position_k"=V2,
         "department_k"=V3) %>% 
  mutate(full_name_k=str_squish(full_name_k),
         department_k=str_squish(department_k))
...
# Для подальших дій потрібно перейти у файл helper.R 
